...
onready var PopupRename = get_parent().find_node("Popup2").find_node("LineEdit")
...
func _on_PopupMenu_id_pressed(id):
	match id:
		0:
			PopupText.get_parent().rect_position = get_viewport().get_mouse_position() + Vector2(-10,-19)
			PopupText.get_parent().popup()
		1:
			if selected_node != null:
				PopupRename.get_parent().rect_position = get_viewport().get_mouse_position() + Vector2(-10,-19)
				PopupRename.get_parent().popup()
		2:
			if selected_node != null:
...
func _on_Popup2_confirmed():
	if selected_node != null:
		selected_node.title = PopupRename.text
		PopupRename.clear()
